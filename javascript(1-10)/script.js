...
function setDate() {
    ...
    dateElement.innerHTML = weekday + ", " + year + "." + day + "." + month + ", " + hours + "/" + minutes + "/" + seconds;
}
...
function countFORMInDOM(node) {
    ...
    if (node.hasChildNodes()) {
        ...
        for (let i = 0; i < childNodes.length; i++) {
            count += countFORMInDOM(childNodes[i]);
        }
    }
    return count;
}
...
function task3(elementId) {
    const elemInDOM = countFORMInDOM(document);
    const element = document.getElementById(elementId);
    element.innerText = `Количество элементов внутри формы: ${elemInDOM}`;
}
...
function task4() {
    ...
    // Добавляем класс, который скрывает блок
    block.classList.add("hidden");

    // Ждем 120 мс и удаляем класс, чтобы блок снова стал видимым
    setTimeout(function() {
      block.classList.remove("hidden");
    }, 120);
}
setInterval(task4, 120);
...
function task7() {
    ...
    message.textContent = "Шоколадки кончились";
...
function validateEmail() {
    ...
    if (!emailPattern.test(email.value)) {
        document.getElementById("emailError").innerHTML = "Enter a valid email address (example@mail.com)";
...
function validatePhone() {
    ...
    if (!phonePattern.test(phone.value)) {
        document.getElementById("phoneError").innerHTML = "Enter a valid phone number (89181234567)";
